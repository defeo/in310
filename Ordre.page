...
On **ordre** sur un ensemble $A$ est une [Relation]() $\preceq\subset{A\times A}$ qui satisfait les propriétés suivantes:
...
- La relation $a|b$ ($a$ divise $b$) est un *ordre partiel*.
...
<img style="width:30%; display:inline-block" src="http://upload.wikimedia.org/wikipedia/commons/e/ea/Hasse_diagram_of_powerset_of_3.svg" alt="Diagramme de Hasse de l'ensemble des parties de l'ensemble à trois éléments."/>
<p style="margin-left:2ex; width:50%; display:inline-block">Cette image a été crée par <a href="http://commons.wikimedia.org/wiki/User:KSmrq">KSmrq</a>. Téléchargée de [Wikimedia Commons](http://en.wikipedia.org/wiki/File:Hasse_diagram_of_powerset_of_3.svg).</p>
...
**Note:** Si l'ordre n'est pas total, l'élément minimal n'est pas nécessairement unique. Considérez par exemple l'ensemble des nombres premiers muni de la relation d'ordre $a|b$.
...
- Les entiers munis de l'ordre $a|b$ forment un *ordre bien fondé* (mais pas un bon ordre).
...
voir [Induction](), [Récursivité]().

Les ordres bien fondés sont à la base du principe d'[Induction]() et de la programmation [récursive](Récursivité).
