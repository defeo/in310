Une relation entre deux [ensembles](Ensemble) $A$ et $B$ est une loi qui associe à chaque élément de $A$ zéro, un ou plusieurs éléments de $B$. Dans ce sens, c'est une généralisation du concept de [Fonction]().
...
$$\mathcal{R}^c = \{(b,a)\in B\times A | (a,b)\in A\times B\}.$$
...
Une relation qui est à la fois réflexive, antisymétrique et transitive est appelée un [Ordre]() (partiel); lorsque elle est aussi totale elle est appelée un [ordre total](Ordre#ordre-total).
...
- La relation sur les naturels $a|b$ ($a$ divise $b$) est réflexive, transitive et antisymètrique, mais pas totale. Elle forme donc un [ordre partiel](Ordre).
